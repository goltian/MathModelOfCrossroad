...
void DataManager::setPortionOfData(
	int row, 
	int column, 
...
void DataManager::writeInfoInFile(
	...
	reportFile.open("../../tables_26.05/eksps_2021_3_potoks/" + nameOfFile + "table.txt", std::ios::trunc);
	std::string str;
	...
	if (minValueIndex == -1) {
		...
		for (int repeatNumber = 0; repeatNumber < repeatsCount; repeatNumber++) {
			for (int cells = 0; cells < CONST_CELLS_COUNT; cells++) {
				str = std::to_string(data[minValueIndex + cells * sizeOfVector + repeatNumber * CONST_CELLS_COUNT * sizeOfVector]);
...
void DataManager::writeInfoInTable(std::string nameOfFile) {
	...
	reportTable.open("../../tables_26.05/eksps_2021_3_potoks/" + nameOfFile + "table.xls", std::ios::trunc);
	std::string str;
	...
	for (int row = 0; row < rowCount; row++) {
		for (int column = 0; column < rowCount; column++) {
			if (row + column < rowCount) {
				...
				} else if ((resultMatrix[index] > 0) && (resultMatrix[index] < resultMatrix[minValueIndex])) {
					minValueIndex = index;
					bestDurationsOfModes = std::make_pair(row + 1, column + 1);
				}
			}
