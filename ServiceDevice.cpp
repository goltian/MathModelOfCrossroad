...
bool ServiceDevice::isCrossroadModelWorksStably() {
	while (true) {
		if ((streamPi1.isStreamNotStable()) || (streamPi2.isStreamNotStable()) || (streamPi3.isStreamNotStable())) {
			return false;
		}
		if ((streamPi1.isStreamStable()) && (streamPi2.isStreamStable()) && (streamPi3.isStreamStable())) {
			return true;
		}
		computeNextMode();
	}
