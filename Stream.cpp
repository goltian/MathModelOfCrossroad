	...
	if (areThereFastRequests()) {
		...
		for (int slowReq = 0; slowReq < slowReqCount; slowReq++) {
			...
			if (slowReq < slowReqCount - 1) {
				timeBetweenFastReq = (timesOfSlowReq[slowReq + 1] - timesOfSlowReq[slowReq]) / (2.0 * fastReqCount);
			} else {
				timeBetweenFastReq = (totalTime + modeDuration - timesOfSlowReq[slowReq]) / (2.0 * fastReqCount);
			}
