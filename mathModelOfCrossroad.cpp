...
int main()
{
	...
	for (int numberOfExp = 0; numberOfExp < CONST_COUNT_OF_EXPERIMENTS; numberOfExp++) {
		...
		DataManager matrixForOneExp(rowCount, 4);
...
#pragma omp barrier
		...
		matrixForOneExp.writeInfoInFile(
			peopleServiceModeDuration, 
			parametersForOne[4], 
			nameOfFile, 
			timeOfWork
...
std::string getNameOfFile(const std::vector<double> parametersForOne, double peopleServiceModeDuration) {
